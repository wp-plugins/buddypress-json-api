...
require_once BUDDYPRESS_JSON_API_FOLDER . '/library/functions.class.php';

class BuddypressJsonAPI_BuddypressRead_Controller {
	...
	************************************************/
    ...
    public function activity_get_activities() {
        ...
        if ($this->pages !== 1) {
            $bpJsonApiParams ['max'] = true;
            $bpJsonApiParams ['per_page'] = $this->offset;
            $iPages = $this->pages;
        }

        $bpJsonApiParams ['display_comments'] = $this->comments;
        $bpJsonApiParams ['sort'] = $this->sort;

        $bpJsonApiParams ['filter'] ['user_id'] = $this->userid;
        $bpJsonApiParams ['filter'] ['object'] = $this->component;
        $bpJsonApiParams ['filter'] ['action'] = $this->type;
        $bpJsonApiParams ['filter'] ['primary_id'] = $this->itemid;
        $bpJsonApiParams ['filter'] ['secondary_id'] = $this->secondaryitemid;
        $iLimit = $this->limit;

        if ($this->pages === 1) {
            $bpJsonApiParams ['page'] = 1;
            if ($iLimit != 0)
                $bpJsonApiParams['per_page'] = $iLimit;
            $aTempActivities = bp_activity_get($bpJsonApiParams);
        ...
        for ($i = 1; $i <= $iPages; $i++) {
            if ($iLimit != 0 && ($i * $bpJsonApiParams['per_page']) > $iLimit) {
                $bpJsonApiParams['per_page'] = $bpJsonApiParams['per_page'] - (($i * $bpJsonApiParams['per_page']) - $iLimit);
                $bLastRun = true;
            }
            $bpJsonApiParams ['page'] = $i;
            $aTempActivities = bp_activity_get($bpJsonApiParams);
            if (empty($aTempActivities['activities'])) {
                if ($i == 1)
                    return $this->error('activity');
    ...
    public function profile_get_profile() {
        ...
        $oReturn = new stdClass();
		$error=0;
		if($this->userid=='' && ($this->username === false || !username_exists($this->username))) {
    ...
    public function messages_get_messages() {
        ...
        $bpJsonApiParams ['box'] = $this->box;
        $bpJsonApiParams ['per_page'] = $this->per_page;
        $bpJsonApiParams ['max'] = $this->limit;

        if (bp_has_message_threads($bpJsonApiParams)) {
    ...
    public function groups_get_groups() {
        ...
        if ($this->username !== false || username_exists($this->username)) {
            $oUser = get_user_by('login', $this->username);
            $bpJsonApiParams ['user_id'] = $oUser->data->ID;
        }

        $bpJsonApiParams ['show_hidden'] = $this->show_hidden;
        $bpJsonApiParams ['type'] = $this->type;
        $bpJsonApiParams ['page'] = $this->page;
        $bpJsonApiParams ['per_page'] = $this->per_page;

        $aGroups = groups_get_groups($bpJsonApiParams);
    ...
    public function __call($sName, $aArguments) {
        if (class_exists("BuddypressJsonAPI_FOR_BUDDYPRESS_FUNCTION") &&
                method_exists(BuddypressJsonAPI_FOR_BUDDYPRESS_FUNCTION, $sName) &&
                is_callable("BuddypressJsonAPI_FOR_BUDDYPRESS_FUNCTION::" . $sName)) {
            try {
                return call_user_func_array("BuddypressJsonAPI_FOR_BUDDYPRESS_FUNCTION::" . $sName, $aArguments);
            } catch (Exception $e) {
                $oReturn = new stdClass();
    ...
    public function __get($sName) {
        return isset(BuddypressJsonAPI_FOR_BUDDYPRESS_FUNCTION::$sVars[$sName]) ? BuddypressJsonAPI_FOR_BUDDYPRESS_FUNCTION::$sVars[$sName] : NULL;
    }
