...
/*
  ...
  Controller description: Buddypress controller for reading & update profile actions
 */

if(!class_exists('JSON_API_BuddypressRead_Controller')) {
	class JSON_API_BuddypressRead_Controller {
		...
		************************************************/
		...
		public function messages_get_messages() {
			...
			$buddypress_json_api_settings ['box'] = $this->box;
			$buddypress_json_api_settings ['per_page'] = $this->per_page;
			$buddypress_json_api_settings ['max'] = $this->limit;

			if (bp_has_message_threads($buddypress_json_api_settings)) {
		...
		public function groups_get_groups() {
			...
			if ($this->username !== false || username_exists($this->username)) {
				$oUser = get_user_by('login', $this->username);
				$buddypress_json_api_settings ['user_id'] = $oUser->data->ID;
			}

			$buddypress_json_api_settings ['show_hidden'] = $this->show_hidden;
			$buddypress_json_api_settings ['type'] = $this->type;
			$buddypress_json_api_settings ['page'] = $this->page;
			$buddypress_json_api_settings ['per_page'] = $this->per_page;

			$aGroups = groups_get_groups($buddypress_json_api_settings);
